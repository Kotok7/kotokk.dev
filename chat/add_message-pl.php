...
if (!$token) {
    echo json_encode(['success'=>false,'error'=>'captcha','error_msg'=>'Potwierdź, że nie jesteś robotem.']);
    exit;
}
...
if (empty($resp['success'])) {
    echo json_encode(['success'=>false,'error'=>'captcha','error_msg'=>'Weryfikacja nie powiodła się. Spróbuj odświeżyć stronę.']);
    exit;
}
...
if (isset($cd[$ip]) && $now - $cd[$ip] < 2) {
    echo json_encode(['success'=>false,'error'=>'delay','error_msg'=>'Odczekaj 2 sekundy przed wysyłaniem kolejnej wiadomości.']);
    exit;
}
$cd[$ip] = $now;
...
if (!empty($_FILES['photo']) && $_FILES['photo']['error']!==UPLOAD_ERR_NO_FILE) {
    ...
    if ($f['error']!==UPLOAD_ERR_OK) {
        echo json_encode(['success'=>false,'error'=>'file','error_msg'=>'Błąd wysyłania zdjęcia.']);
        exit;
    }
    if ($f['size']>20*1024*1024) {
        echo json_encode(['success'=>false,'error'=>'file_size','error_msg'=>'Zdjęcie max 20 MB.']);
        exit;
    }
    ...
    if (!isset($allowedImg[$mime])) {
        echo json_encode(['success'=>false,'error'=>'file_type','error_msg'=>'Dozwolone są tylko formaty JPG, PNG i GIF.']);
        exit;
    }
    $ext = $allowedImg[$mime];
...
if (!empty($_FILES['voice']) && $_FILES['voice']['error']!==UPLOAD_ERR_NO_FILE) {
    $f = $_FILES['voice'];
    if ($f['error']!==UPLOAD_ERR_OK) {
        echo json_encode(['success'=>false,'error'=>'voice_file','error_msg'=>'Błąd uploadu audio.']);
        exit;
    }
    if ($f['size']>20*1024*1024) {
    ...
    if (!isset($allowedAudio[$mime])) {
        echo json_encode(['success'=>false,'error'=>'voice_type','error_msg'=>'Dozwolone są tylko formaty MP3, OGG, WAV, WEBM i MP4.']);
        exit;
    }
