...
$ipFile = __DIR__ . '/ip_cooldowns_pl.json';
$cooldown = 3600;
$ip = $_SERVER['REMOTE_ADDR'] ?? 'unknown';
$sentence = isset($_POST['sentence']) ? trim($_POST['sentence']) : '';
if ($sentence === '') {
    echo json_encode(['ok' => false, 'error' => 'Puste zdanie']);
    exit;
}
if (mb_strlen($sentence) > 500) {
    echo json_encode(['ok' => false, 'error' => 'Za długie zdanie (max 500 znaków)']);
    exit;
}
...
$fh = fopen($ipFile, 'c+');
if ($fh === false) {
    echo json_encode(['ok' => false, 'error' => 'Nie można otworzyć pliku cooldown']);
    exit;
}
if (!flock($fh, LOCK_EX)) {
    fclose($fh);
    echo json_encode(['ok' => false, 'error' => 'Błąd blokady']);
    exit;
}
rewind($fh);
...
if ($fh2 === false) {
    echo json_encode(['ok' => false, 'error' => 'Nie można otworzyć pliku opowieści']);
    exit;
}
