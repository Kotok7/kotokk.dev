...
<head>
  ...
  <meta name="description" content="Simple real-time web chat with image and voice message sharing. Also features a live active user counter!">
...
<header>
  <div class="header-content">
    <div id="online-container">
      <span>Users online:</span> <span id="online-count">–</span>
    </div>
    <h1>Live Chat</h1>
    <h1 style="font-size:90%;">English chat</h1>
    <p>Created by @kotokk_dev</p>
    <a href="index.php" class="language-btn">Wejdź na Polski czat</a>
  ...
  <section class="leave-message" id="leave-message">
    <form id="chat-form" enctype="multipart/form-data">
      ...
      <div id="reply-preview" style="display:none;margin-bottom:8px;font-style:italic;color:#555;">
        ...
        <button type="button" id="cancel-reply" style="margin-left:8px;">Cancel</button>
      </div>
...
<footer>
  <p>&copy; 2025 kotokk.dev<br>My website: <a href="https://kotokk.dev" target="_blank">kotokk.dev</a></p>
</footer>
...
toggleVoiceBtn.addEventListener('click', ()=>{
  if (voiceFileLabel.style.display==='none') {
    ...
    clearAudioPreview(); audioBlob=null; toggleVoiceBtn.textContent='Record';
  } else {
    voiceFileLabel.style.display='none';
    startBtn.style.display='inline-block'; startBtn.disabled=false;
    stopBtn.style.display='inline-block'; stopBtn.disabled=true;
    deleteBtn.style.display='none';
    clearAudioPreview(); audioBlob=null; toggleVoiceBtn.textContent='Upload file';
  }
});
...
startBtn.addEventListener('click', async ()=>{
  ...
  try {
    ...
    recordTimer = setInterval(()=>{
      ...
      recordTimerSpan.textContent = `Recording: ${elapsed}s`;
      if (elapsed >= 60) {
        mediaRecorder.stop();
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  async function fetchOnlineCount(){
    try {
      const res = await fetch('online_counter.php', { cache:'no-store' });
  ...
  function attachReplyHandlers(data){
    msgWrapper.querySelectorAll('.reply-btn').forEach(btn=>{
      btn.onclick = ()=>{
        ...
        replyInput.value = id;
        replyTextEl.textContent = `Replying to ${orig.nick}: ${orig.text}`;
  ...
  form.addEventListener('submit', async e=>{
    e.preventDefault();
    if(!turnstilePassed){
      document.getElementById('form-error').textContent = 'Please verify you are not a robot.';
    ...
    const res = await fetch('add_message.php', { method:'POST', body:fd });
    const d = await res.json();
    if(d.success){
      form.reset();
      replyInput.value = '';
